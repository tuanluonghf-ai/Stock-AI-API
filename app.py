...
def _extract_d_block(text: str) -> str:
    # Extract from "D." to end; fallback to full text if not found.
...
def _grab_number(block: str, label_patterns: List[str]) -> float:
    """
    Extract first float number after matching label pattern(s).
    Accepts formats: 5.996, 5,996, 5.996%, 5.996 %
    """
...
def validate_section_d(text: str, primary: Dict[str, Any]) -> bool:
    ...
    # probability check (soft substring)
...
"""
    ...
    try:
        # GUARD-D: validate section D; retry with lower temperature if mismatch
