# ============================================================
# INCEPTION v4.9.2 | Strategic Investor Edition
# app.py — Streamlit + GPT-4 Turbo
# Author: INCEPTION AI Research Framework
# Purpose: Technical–Fundamental Integrated Research Assistant
# CHANGELOG:
# v4.9:   Refactored A.1 MA Trend (Facts over Conclusions)
# v4.9.1: Refactored A.2 RSI (Added Series/Deltas, removed Labels)
# v4.9.2: Refactored A.3 MACD (Added Hist Series/Streaks, removed Labels)
# ============================================================
...
# ============================================================
st.set_page_config(page_title="INCEPTION v4.9.2",
                   layout="wide",
                   page_icon="🟣")
...
def load_hsc_targets(path: str = HSC_TARGET_PATH) -> pd.DataFrame:
    ...
    # Simple heuristic to fix units if needed
...
def compute_rsi_features(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Step 2 change (A.2 RSI only):
    - Remove "hard judgement" wording in State (e.g., Overbought/Oversold).
    - Keep backward-compatible keys: Value, State, Direction, Divergence.
    - Add raw context so GPT can decide "good/bad" by regime: series, deltas, streaks, extremes.
    """
    if df.empty:
        return {}
    ...
    # Direction over last ~5 bars (kept, but computed more robustly)
    ...
    # Neutral zone label (no "good/bad" words)
    # Keep key name "State" for compatibility with prompt.
    zone = "N/A"
    ...
    # Streaks: consecutive bars above 70 / below 30 ending at last bar
    ...
    streak_above70 = _streak((rsi >= 70).tail(60))  # cap window for speed
    streak_below30 = _streak((rsi <= 30).tail(60))
    
    # Divergence (kept as-is for now)
    div = _detect_divergence_simple(df["Close"], df["RSI"], lookback=60)
    
    return {
        # Backward-compatible keys used by prompt
        "Value": last_rsi,
        "State": zone,          # replaced judgement labels with neutral zone
        "Direction": direction,
        "Divergence": div,
        
        # Added raw context for richer GPT interpretation (non-breaking additions)
        "Delta5": delta5,
        "RSI_Series_6": tail6,
        "Max20": rsi_max20,
        "Min20": rsi_min20,
...
def compute_macd_features(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Step 3 change (A.3 MACD only):
    - Keep backward-compatible keys: Value, Signal, Hist, State, Cross, ZeroLine, HistState, Divergence.
    - Reduce "hard conclusion" by using neutral zones / factual context.
    - Add raw context: recent hist series, deltas, streaks, and distance between MACD & Signal.
    """
    if df.empty:
        return {}
    ...
    # Neutral relationship label (avoid "Bull/Bear" wording but keep key "State")
    # State now represents relationship MACD vs Signal.
    state = "N/A"
    diff_ms = np.nan
    if pd.notna(last_m) and pd.notna(last_s):
        diff_ms = last_m - last_s
        if diff_ms > 0:
            state = "MACD_Above_Signal"
        elif diff_ms < 0:
            state = "MACD_Below_Signal"
        else:
            state = "MACD_Near_Signal"
            
    # Last crossover fact (kept)
    cross = _find_last_cross(df["MACD"], df["MACDSignal"], lookback=30)
    
    # Zero-line location (kept but factual)
    zero = "N/A"
    if pd.notna(last_m):
        if last_m > 0.0:
            zero = "Above"
    ...
    # Histogram recent context
    h_tail6 = []
    if not hist.empty:
        h_tail6 = hist.tail(6).tolist()
        h_tail6 = [(_safe_float(x) if pd.notna(x) else np.nan) for x in h_tail6]
        
    # Histogram state (kept for compatibility, but still factual)
    # Uses last 3 histogram values to describe expansion/contraction and sign.
    hist_state = "N/A"
    if len(hist) >= 4:
        ...
        if pd.notna(h0) and pd.notna(h1) and pd.notna(h2):
            if h0 >= 0 and h1 >= 0:
                if (h0 > h1 > h2):
                    hist_state = "Expanding_Positive"
                elif (h0 < h1 < h2):
                    hist_state = "Contracting_Positive"
                else:
                    hist_state = "Mixed_Positive"
            elif h0 < 0 and h1 < 0:
                if (h0 < h1 < h2):
                    hist_state = "Expanding_Negative"
                elif (h0 > h1 > h2):
                    hist_state = "Contracting_Negative"
                else:
                    hist_state = "Mixed_Negative"
            else:
                hist_state = "Sign_Flip"
                
    # Streaks: consecutive histogram bars above 0 / below 0 ending at last bar
    def _streak(cond_series: pd.Series) -> int:
        cnt = 0
        for v in reversed(cond_series.tolist()):
            if bool(v):
    ...
    # Delta over ~5 bars for MACD line (momentum change)
    delta5 = np.nan
    if len(macd_v) >= 6:
    ...
    return {
        ...
        # Added raw context (non-breaking additions)
...
# ============================================================
st.markdown("<h1 style='color:#A855F7; margin-bottom:6px;'>INCEPTION v4.9.2</h1>", unsafe_allow_html=True)
st.divider()
...
st.markdown(
    ...
    Phiên bản 4.9.2 | Engine GPT-4 Turbo
    </p>
    """,
    unsafe_allow_html=True
)
