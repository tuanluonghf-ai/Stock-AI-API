...
# ============================================================
# STABILITY LAYER (UI CONSUMERS)
# ============================================================

def _get_stable_action(ap: Dict[str, Any], fallback: str = "") -> str:
    """Return Decision action that UI should display.

    Priority:
      1) DecisionStabilityPack.stable_action (if present)
      2) DecisionPack.action
      3) fallback
    """
    try:
        dsp = ap.get("DecisionStabilityPack") or {}
        if isinstance(dsp, dict):
            s = _safe_text(dsp.get("stable_action")).strip().upper()
            if s:
                return s
    except Exception:
        pass
    try:
        ...
        if isinstance(dp, dict):
            s = _safe_text(dp.get("action")).strip().upper()
            if s:
                return s
    except Exception:
        pass
    return _safe_text(fallback).strip().upper() or ""

def _get_plan_state(ap: Dict[str, Any]) -> str:
    """Return plan_state ∈ {ACTIVE, PAUSED, INVALIDATED} if available."""
    try:
        psp = ap.get("PlanStabilityPack") or {}
        if isinstance(psp, dict):
            return _safe_text(psp.get("plan_state")).strip().upper()
    except Exception:
        pass
    return ""

def _get_anchor_phrase(ap: Dict[str, Any]) -> str:
    try:
        nap = ap.get("NarrativeAnchorPack") or {}
        if isinstance(nap, dict):
            return _safe_text(nap.get("anchor_phrase")).strip()
    except Exception:
        pass
    return ""
